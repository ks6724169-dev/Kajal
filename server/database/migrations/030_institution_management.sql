...
ALTER TABLE campuses ADD COLUMN IF NOT EXISTS tenant_id UUID;
...
DROP POLICY IF EXISTS tenant_isolation_policy ON campuses;
CREATE POLICY tenant_isolation_policy ON campuses FOR ALL USING (
    tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::UUID
);
