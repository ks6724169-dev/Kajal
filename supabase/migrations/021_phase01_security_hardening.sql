-- Migration 021: Phase 01 Security Hardening
-- Remediates Critical vulnerability in get_owner_dashboard_stats where client-submitted p_tenant_id is blindly trusted.
-- Secures search path and strictly enforces auth role validation against the resolved tenant context.
...
BEGIN
    -- Determine authoritative tenant context of the logged-in user
    v_auth_tenant_id := public.get_auth_user_tenant_id();
    v_auth_role := public.get_auth_user_role();

    -- Resolve requested tenant from parameter
    ...
    -- If p_tenant_id is supplied, we MUST verify that the authenticated user actually owns/belongs to it,
    -- or is a super_admin. Otherwise, we ignore it and default to the user's authentic v_auth_tenant_id.
    IF v_tenant_id IS NOT NULL THEN
        IF v_auth_role != 'super_admin' AND (v_auth_tenant_id IS NULL OR v_auth_tenant_id != v_tenant_id) THEN
            -- Malicious request or tenant mismatch. Restrict access and coerce to user's real tenant ID.
    ...
    -- Default fallback UUID for system organization if somehow still null
...
-- Grant execution permissions securely
REVOKE EXECUTE ON FUNCTION get_owner_dashboard_stats(TEXT, VARCHAR) FROM anon;
