...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'landing' | 'login' | 'app'>(() => {
    ...
    if (path === '/login' || path === '/auth/login') return 'login';
    if (path === '/app' || path === '/dashboard' || path === '/workspace' || path.startsWith('/owner/')) return 'app';
    return 'landing';
  });
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      const protectedRoutes = ['/app', '/dashboard', '/workspace', '/command-center'];
      if (protectedRoutes.includes(route) || route.startsWith('/owner/')) {
        setCurrentView('login');
        setRoute('/login');
      }
    }
